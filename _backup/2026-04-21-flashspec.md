...
在传统的首先中，durring the verify stage, the target model 通过一次forward计算出所有draft tokens的概率。然后去比较$u$和 $\min (1, \frac{p(x){q(x)}})$来判断draft tokens是否被接收，其中$u\in \mathcal{U}(0,1)$服从0到1之间的均匀分布。
